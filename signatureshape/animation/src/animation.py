...
class Animation(object):
    ...
    #            cPickle.dump(self._frames, f)
    #
    #    def load_from(self, filename):
    #        self._index = 0
    #        self._offset = zeros(3)
    #        with open(filename, 'r') as f:
    #            self._frames = cPickle.load(f)
    #
    ...
    def step(self, continuous=False):
        ...
        if self._index >= len(self._frames) - 1:
            ...
            if continuous:
                self._offset += (
                    self._frames[-1]["root"][0:3] - self._frames[0]["root"][0:3]
                )
        else:
            self._index += 1
    ...
    def get_channel_indices(self):
        ...
        s = 0
        for c in list(self._frames[0].keys()):
            channel_indices[c] = s
    ...
    def to_numpy_array(self):
        ...
        channels = list(self._frames[0].keys())
