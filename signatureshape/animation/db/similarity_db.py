...
def insert_signature_distance(animation_id1, animation_id2, signature_distance):
    ...
    if check_animation_id(animation_id1, animation_id2):
        ...
        cursor.execute(
            """
            INSERT INTO
            similarity(animation_id1, animation_id2, signature_distance)
            VALUES (?, ?, ?);
        """,
            (animation_id1, animation_id2, signature_distance),
        )
    connection.commit()
    cursor.execute("SELECT count(*) FROM similarity;")
    return int(cursor.fetchone()[0])
...
def insert_se3_signature_distance(animation_id1, animation_id2, se3_signature_distance):
    connection, cursor = set_up()
    if check_animation_id(animation_id1, animation_id2):
        cursor.execute(
            """
            UPDATE similarity
            SET se3_signature_distance=?
            WHERE
            animation_id1=? and animation_id2=?;
        """,
            (se3_signature_distance, animation_id1, animation_id2),
        )
        print("Update set:", animation_id1, animation_id2, se3_signature_distance)
    else:
        cursor.execute(
            """
            INSERT INTO
            similarity(animation_id1, animation_id2, se3_signature_distance)
            VALUES (?, ?, ?);
        """,
            (animation_id1, animation_id2, se3_signature_distance),
        )
        print("Insert set:", animation_id1, animation_id2, se3_signature_distance)
    connection.commit()
    cursor.execute("SELECT count(*) FROM similarity;")
    return int(cursor.fetchone()[0])
