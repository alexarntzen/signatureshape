...
from .db.similarity_db import (
    check_animation_id,
    insert_similarity,
    insert_signature_distance,
    insert_se3_signature_distance,
)
...
def fetch_animations(
    ...
    animations = get_animation(
        animation_id, file_name, description, subject_file_name, subject_fkey
    )
    # if fetchone
    ...
    arr = []
    for i in tqdm(range(min(len(animations), max))):
        try:
...
def save_similarity(animation_id1, animation_id2, distance, dp_distance, size1, size2):
    ...
    if not check_already_calucated(animation_id2, animation_id1):
        total = insert_similarity(
            animation_id2, animation_id1, distance, dp_distance, size1, size2
        )
        number_calculated += 1
...
def save_signature_se3_distance(animation_id1, animation_id2, se3_signature_distance):
    insert_se3_signature_distance(animation_id1, animation_id2, se3_signature_distance)
    insert_se3_signature_distance(animation_id2, animation_id1, se3_signature_distance)
