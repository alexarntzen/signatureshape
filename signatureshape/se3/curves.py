...
from se3.transformations import concatenate_curve, SRVT, inverse_SRVT

# from so3.transformations import interpolate as tf_interpolate
from .helpers import norm

# TODO: delete because it is wrong and not used
...
def distance(c0, c1, I0=None, I1=None):
    ...
    q0 = concatenate_curve(SRVT(c0, I0))
    q1 = concatenate_curve(SRVT(c1, I1))
...
def L2_metric(q0, q1, I0, I1):
    if array_equal(I0, I1):
        return sqrt(
            sum(
                (I0[k + 1] - I0[k]) * (norm(q0[k] - q1[k]) ** 2)
                for k in range(I0.shape[0] - 1)
            )
        )
