import pylab as pl
from se3.helpers import assemble_SE3
...
def convert_to_SE3(skeleton, frame, filter_noisy_channels=False):
    def convert(node):
        if node == "root":
        ...
        try:
            for dof, val in zip(cbone.dof, frame[node]):
                ...
                if dof == "rx":
                    ...
                    R = Rz(val)
                M = pl.dot(R, M)
            # include distance. Not sure if this is accurate hmm
            # TODO: verify this is correct and deuglyfiy the code
            t = cbone.direction * cbone.length
            M = assemble_SE3(M[0:3, 0:3], t)
...
def animation_to_SE3(skeleton, animation):
    ...
    for frame in animation.get_frames():
        frames.append(convert_to_SE3(skeleton, frame, True))

    # Convert into numpy arrays
    channels = pl.zeros((len(bone_names), len(frames), 4, 4))
    ...
    for key in animation.channel_order:
        ...
        for j, frame in enumerate(frames):
            channels[i, j, :, :] = frame[key]

    for i in range(channels.shape[0]):
        for j in range(channels.shape[1]):
            if pl.array_equal(channels[i, j, :, :], pl.zeros((4, 4))):
                channels[i, j, :, :] = pl.eye(4)
