...
def concatenate_metric(c0, c1, s):
    ...
    q0 = concatenate_curve(right_log_se3(c0, I0))
    q1 = concatenate_curve(right_log_se3(c1, I1))

    u = np_concatenate((q0[:-1], flip(q1[:-1], axis=0)), axis=0)
    v = np_concatenate((q1[:-1], flip(q0[:-1], axis=0)), axis=0)

    Iu = np_concatenate(
        (linspace(0, 0.5, q0.shape[0]), linspace(0.5, 1, q1.shape[0])[1:]), axis=0
    )
    Iv = np_concatenate(
        (linspace(0, 0.5, q1.shape[0]), linspace(0.5, 1, q0.shape[0])[1:]), axis=0
    )

    x = lift_piece_wise_constant(u, Iu)
    y = lift_piece_wise_constant(v, Iv)
    return np_norm(log_signature(u, s)) + np_norm(log_signature(v, s))
