...
print("Load data")
data0 = fetch_animations(1, file_name="39_02.amc")
data1 = fetch_animations(1, file_name="35_26.amc")

print("Parse data")
s0, a0, d0 = unpack(data0)
c0d = animation_to_SO3(s0, a0)

s1, a1, d1 = unpack(data1)
...
for s in lin:
    c = curves.interpolate(c0d, c1d, s)
    ...
    if s == 0:
        diff_c0d_dp.append(0.0)
        d1_dp = curves.dynamic_distance(c1d, c)
        diff_c1d_dp.append(d1_dp if not np.isnan(d1_dp) else 0)
        continue
    if s == 1:
        d0_dp = curves.dynamic_distance(c0d, c)
        diff_c0d_dp.append(d0_dp if not np.isnan(d0_dp) else 0)
        diff_c1d_dp.append(0)
        continue
...
plt.plot(lin, diff_c0d_dp, label="difference compared to c0")
plt.plot(lin, diff_c1d_dp, label="difference compared to c1")
plt.legend()
plt.show()
