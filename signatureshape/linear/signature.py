from numpy import insert, zeros, sqrt, flip, concatenate as np_concatenate
...
def concatenate(x, y):
    return np_concatenate((x, y[::-1]), axis=0)


def curve_concat_log_metric(x, y, s):
    return max(logsig(concatenate(x, y), s))
...
def CC_distance(sig0, sig1, words=None, word_index=None, d=None, k=None):
    ...
    f = lambda r, i: pow(factorial(i) * sqrt(r), 1.0 / i)
...
def distance(a, b, k, d):
    ...
    sig0 = signature(a, k)
    sig1 = signature(b, k)
    return CC_distance(sig0, sig1, words, word_index, d=d, k=k) + CC_distance(
        sig1, sig0, words, word_index, d=d, k=k
    )
